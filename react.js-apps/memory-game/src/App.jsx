...
export function App() {
  ...
  //*estados para controlar el modal de victoria
  const [finishedItems, setFinishedItems] = useState([])
  const [victoryValue, setVictoryValue] = useState(false)
  ...
  //*Disparar por primera vez el shuffle para empezar el juego
  useEffect(() => {
    shufflePictures()
  }, [])
  ...
  useEffect(() => {
    ...
    if (choiceOne && choiceTwo) {//* de esta manera la comparación se hace solo cuando ambas choices tengan un valor y no en cada click
      ...
      if (choiceOne.src === choiceTwo.src) {
        ...
        setFinishedItems([...finishedItems, choiceOne])
        // setChoiceOne(null)
        // setChoiceTwo(null)
        // setDisabled(false)
        resetTurn()
  ...
  useEffect(() => {
    ...
    if (finishedItems.length === boardPictures.length) {
        setVictoryValue(true)
    }
  }, [finishedItems])
  ...
  return (
    <>
      ...
      <VictoryModal moves={move} victoryValue={victoryValue} setVictoryValue={setVictoryValue}/>
